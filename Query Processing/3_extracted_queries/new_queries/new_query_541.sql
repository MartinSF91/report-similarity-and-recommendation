/* user=Piratla Manikanta reportPath= report= queryName=Price Not NULL REMOTE_ADDR= SERVER_NAME=cognosanalytics.apps.ikea.com requestID=CvyqvMvwysyv4whMdysllGM99qGGswhd8dwMqGsC */
SELECT TRIM(BOTH
            FROM "L62T10"."PARTREV"),
       TRIM(BOTH
            FROM "L62T10"."PARTNO"),
...
FROM
  (SELECT "L62T1"."PARTNO" AS "PARTNO",
   ...
   FROM "L62T1" "L62T1"
   WHERE NOT (TRIM(BOTH
                   FROM "L62T1"."DIVCODE") IN ('----'))) "L62T10" /* user=Piratla Manikanta reportPath= report= queryName=Price Not NULL REMOTE_ADDR= SERVER_NAME=cognosanalytics.apps.ikea.com requestID=CvyqvMvwysyv4whMdysllGM99qGGswhd8dwMqGsC */
