...
"SD_Weekly_Adjustments_Gross_LW":

SQL returns all SD gross inventory adjustments from the previous workweek (Sun - Sat)
...
WHERE x08.divcode = '019'
  ...
  AND x08.datreg <
    (SELECT next_day(trunc(sysdate), 'Sondag') - 7 datend
     FROM DUAL) --x08.datreg >= (SELECT next_day(trunc(sysdate), 'SUNDAY') - 14 datstart FROM DUAL) AND
