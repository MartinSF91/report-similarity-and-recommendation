...
INNER JOIN
  (SELECT logguser,
          ...
          min(l16seqno) seq,
          min(datreg) "1st Transaction",
          ROUND((min(datreg)-(sysdate- NUMTODSINTERVAL(1, 'HOUR')- NUMTODSINTERVAL(00, 'MINUTE')))*1440, 1) "_Minutes from shift start" ,
          '' "_Comments"
   FROM ast052.l16t3
   ...
   WHERE l16lcode IN(1,
                     ...
                     61,
                     26)
     AND datreg>(sysdate- NUMTODSINTERVAL(1, 'HOUR')- NUMTODSINTERVAL(00, 'MINUTE'))
   GROUP BY logguser,
            trim(s90t1.email),
            trim(s90t1.fname),
            trim(s90t1.lname))T1 ON l16t3.l16seqno=T1.seq
AND l16t3.logguser=T1.logguser
WHERE l16seqno=T1.seq
ORDER BY "_Minutes from shift start" DESC